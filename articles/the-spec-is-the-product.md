...
created: 2026-07-22
status: published
published: https://chronograph.rosenlidholm.se/the-spec-is-the-product
---
...
The diagnosis has allies in an entirely different tradition. Gîrba and Wardley's [Rewilding Software Engineering](https://medium.com/feenk/rewilding-software-engineering-900ca95ebc8c) argues that manually produced views of a system ["constitute beliefs rather than accurate engineering tools"](https://book.gtoolkit.com/mapping-moldable-development-59974yztvzadec0whbcsp3np5), and their cure, [Moldable Development](https://moldabledevelopment.com/) (practiced in their Glamorous Toolkit), is deterministic derivation at read time: regenerate every view from the system on demand, so it cannot be stale. This article is the symmetric case on the write side: derive the system from the spec at build time, so the representations cannot drift.
...
If act three still sounds exotic, look at what is already running in your service. Wolverine, the message-bus half of the Critter Stack, reads your handler signatures and generates the code that wraps them: the middleware chain, the transaction, the outbox. Its `TypeLoadMode` setting is the three acts as an enum. `Dynamic` compiles the adapters in memory at first use and nothing touches disk. `Static`, the production recommendation, has you run `codegen write` first, and the files land in `Internal/Generated` under your project, where the question of whether to commit them is yours to answer, and the answer is this whole section. System.Text.Json makes the same offer one layer down: the reflection serializer derives the contract at runtime, a `JsonSerializerContext` derives the same contract at build, and one attribute picks. Same transformation, same output, two times to run it. Anyone who has used either has already accepted code they did not write, derived from a declaration they did, verified by the compiler or by a failure at boot. The generator in this article does nothing those two do not. The only thing that moved is the input, from a method signature to a domain spec.
...
**Determinism has to be proven, not assumed.** A generator with an unnoticed nondeterminism — dictionary ordering, culture-sensitive formatting, a timestamp — turns "build step" back into "review event" silently. The contract `artifact == f(spec, generator)` is only worth what the round-trip test enforcing it is worth. System.Text.Json is the reassuring precedent and the warning at once: nobody writes a test proving its reflection path and its generated path agree, because the framework proved it once for everyone, and a private generator has no such luxury.
