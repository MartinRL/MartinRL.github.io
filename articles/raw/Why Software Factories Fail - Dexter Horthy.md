...
Dexter Horthy
Dexter Horthy
founder @ humanlayer - multiplayer software factory control plane
...
Edit: I actually have to give an honorable mention here to SlopCodeBench from Gabe Orlanski's lab at UW - It's a deeply unsaturated benchmark (17% strict pass rate for opus 4.6) and it specifically measures a model's ability to maintain a codebase over time.

To figure out why this happens, I want to zoom out to the first great coding agent.
...
starts from a base commit -- the repo checked out to the moment right before that fix landed

the bug report - in this case 'zip_command': undefined method 'empty?' for nil:NilClass
...
Run the whole suite: the existing zip tests (PASS_TO_PASS) plus the new one (FAIL_TO_PASS) to see if they both pass
...
The one benchmark I do think is really interesting, as noted before, is SlopCodeBench - it's worth a read.
