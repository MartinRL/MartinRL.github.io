...
1) Re-scope `emlang-web` from "wireframe generator" to *abstract-UI generator*: screens, bindings, navigation, form scaffolds, tokens file, generated Playwright specs per GWT slice, and generated aria snapshot baselines per screen; and since Roslyn source generators emit C# only (no HTML, no Blazor), keep it a pre-build CLI tool in the Grpc.Tools/NSwag mold.
   
2) Add a `tokens` block (DTCG-compatible) to the emlang dialect so styling vocabulary lives in the spec; do not add layout syntax, that is the MBUI tarpit. 
