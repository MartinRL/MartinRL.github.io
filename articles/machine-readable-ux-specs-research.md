...
1) Re-scope `emlang-web` from "wireframe generator" to *abstract-UI generator*: screens, bindings, navigation, form scaffolds, tokens file, and generated Playwright specs per GWT slice; and since Roslyn source generators emit C# only (no HTML, no Blazor), keep it a pre-build CLI tool in the Grpc.Tools/NSwag mold. 
...
3) Adopt the visual-baseline lockfile: Playwright `toHaveScreenshot` per generated scenario, baselines committed, agent restyling gated on baseline diffs. 
