...
The mechanism had a signature: a deterministic *generator*, running at *build time*, verified by the *compiler*. A fake event added to the spec fails the next build with CS8795 and the name of the method a human now owes; that seam was the whole argument.
...
Part one lived entirely in the inner ring. An event model states what the system *does*: commands, events, views, scenarios. It deliberately says nothing about what anyone *sees* — which fields matter most on which screen, for whom, in what order, under what name, in what typeface. That is the Experience Model's ring: how humans inhabit the system. Civil engineering splits these professions for a reason; the structural engineer certifies the building will not fall down, the architect ensures people can live in it.
...
Honesty about the second property, though, because it genuinely changed: **what verifies is no longer the compiler.** No CS8795 fires when a surface references a view that doesn't exist. Three oracles stand where the compiler stood:
